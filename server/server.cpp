...
const int TIMEOUT = 300000; // Timeout Threshold
const int TIMER_STOP = -100;
using namespace std;
...
int time_count = 0;
int protocol = 1;    // 0 for stop and wait, 1 for selective repeat

// Acknowledgment Packet Struct
struct ack_packet
{
    uint16_t cksum; // bonus part 
    uint16_t len;
    uint32_t ackno;
};
...
int server_port = 0;
int max_window_size; // Maximum Window size
int seed; // random seed
float p;  // Probability for packet loss.
int cur_window_size=2;  // Current Window size
...
int ssth=50; // current ssthreshold
...
{
    logFile.open("log.txt"); // open Log file to log congestion window
    read_input_file();  // read server configuration from server.in
...
void run()
{
    signal(SIGSEGV, sig_func); // handle segmentation fault gracefully.
    socklen_t alen;
    int sockoptval = 1;
